...
void traceroute(char *dest)
{
     ...
     * Handled by 'ttl' and 'retry' for loops
     */
    for (int ttl = 1; ttl <= MAX_HOPS; ttl++)
    {
        ...
        for (int retry = 0; retry < MAX_RETRY; retry++)
        {
            ...
            if (sendto(sockfd, send_buf, sizeof(icmpheader), 0, (struct sockaddr *)&send_addr, sizeof(send_addr)) <= 0)
            {
            ...
            else if (ret > 0)
            {
                ...
                int len = recvfrom(sockfd, recv_buf, sizeof(recv_buf), 0, (struct sockaddr *)&recv_addr, &addrlen);
                if (len <= 0)
                {
                ...
                if (len >= 2 * (sizeof(ipheader) + sizeof(icmpheader)) && recv_icmphdr->icmp_type == ICMP_TIME_EXCEEDED)
                {
                    icmpheader *orig_icmphdr = (icmpheader *)(recv_buf + sizeof(ipheader) * 2 + sizeof(icmpheader));
                    if (orig_icmphdr->icmp_seq == ttl && orig_icmphdr->icmp_id == getpid())
                    {
                        printf("%s\n", inet_ntoa(recv_iphdr->iph_sourceip));
                        break;
                    }
                }
                else if (recv_icmphdr->icmp_type == ICMP_ECHO_REPLY && recv_icmphdr->icmp_id == getpid())
                {
                    printf("%s\n", inet_ntoa(recv_iphdr->iph_sourceip));
                    exit(0);
                }
            ...
            /** TODO: 6
            *   NOTE: This is handled above by:
            *         'ttl' for loop - automatically increments ttl for each iteration
            *         'RETRY' for loop - handles retries for each ttl and terminates if retry = MAX_RETRY, moving to next hop
            */
        }
    }
    close(sockfd);
