...
private:
	...
	std::condition_variable /*data available*/ pop_ready;	// cond var for indicating buffer is not full; makes producers wait if buffer is full (overflow)
	std::condition_variable /*room available*/ push_ready;	// cond var for indicating buffer is not empty; makes consumers wait if buffer is empty (underflow)
