...
void process_newchannel_request (FIFORequestChannel* _channel) {
	cout << "Server: Received a new channel request." << endl; // debug
	nchannels++;
	string new_channel_name = "data" + to_string(nchannels) + "_";
	char buf[30];
	strcpy(buf, new_channel_name.c_str());
	cout << "Server: Generated new channel name: " << buf << endl; // debug
	_channel->cwrite(buf, new_channel_name.size()+1);
	cout << "Server: Sent new channel name to client: " << buf << endl; // debug
	...
	cout << "Server: Spawned a new thread to handle requests for the new channel." << endl; // debug
	thread_for_client.detach();
	cout << "Server: New channel created and thread started." << endl; // debug
}
...
void populate_file_data (int person) {
	...
	if (ifs.fail()){
		EXITONERROR("Server: Data file: " + filename + " does not exist in the BIMDC/ directory");
	}
...
void process_file_request (FIFORequestChannel* rc, char* request) {
	...
	if (f.length > buffercapacity) {
		cerr << "Server: Client is requesting a chunk bigger than server's capacity" << endl;
		cerr << "Server: Returning nothing (i.e., 0 bytes) in response" << endl;
...
void handle_process_loop (FIFORequestChannel *channel) {
	...
	cout << "Server: Started processing loop for a channel." << endl;
	char* buffer = new char[buffercapacity];
	if (!buffer) {
		EXITONERROR ("Server: Cannot allocate memory for server buffer");
	}

	while (true) {
		int nbytes = channel->cread(buffer, buffercapacity);
		if (nbytes < 0) {
			cerr << "Server: Client-side terminated abnormally" << endl;
			break;
		}
		else if (nbytes == 0) {
		...
		if (m == QUIT_MSG) {  // note that QUIT_MSG does not get a reply from the server
		    cout << "Server: Received QUIT_MSG. Preparing to close channel." << endl; // debug
			cout << "Server: Client-side is done and exited" << endl; // original
			break;
		}
	...
	delete[] buffer;
	delete channel;
	cout << "Server: Exiting processing loop for the channel." << endl; // debug
