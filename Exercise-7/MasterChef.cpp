...
static void timerHandler(int sig, siginfo_t *si, void *uc)
{
	...
	Step *comp_item = (Step *)si->si_value.sival_ptr; // enabled by SA_SIGINFO in main()

	/* TODO This Section - 2 */
	// Officially complete the step using completedSteps and completeCount
	comp_item->PrintComplete();				  // print step completion
	completedSteps->push_back(comp_item->id); // add step to completed list
	completeCount++;						  // incrememnt steps completed

	// Ready to remove that dependency, call the trigger for the appropriate handler
	raise(SIGUSR1); // signal process to call RemoveDepHandler
...
void RemoveDepHandler(int sig)
{
	...
	{
		sig = completedSteps->back();
		completedSteps->pop_back();

		// remove step as dependency to allow other steps to proceed
		recipeSteps->RemoveDependency(sig);
	}
...
int main(int argc, char **argv)
{
	...
	struct sigaction sa;
	sa.sa_flags = SA_SIGINFO;		// enables timeHandler to take siginfo_t ptr as second arg
	sa.sa_sigaction = timerHandler; // set timerHandler as sig handler
	sigemptyset(&sa.sa_mask);		// initialize and clear mask
	...
	// Using signal() instead of sigaction() because this case is a simple signal-to-handler assignment
	signal(SIGUSR1, RemoveDepHandler); // set behavior of SIGUSR1 to call RemoveDepHandler
	...
	{
		// create vector and fill with ready steps
		vector<Step *> readySteps = recipeSteps->GetReadySteps();

		// iterate through readySteps
		for (Step *s : readySteps)
		{
			if (!s->running)
			{
				// start timer for current step if not running
	...
	// memory clean up - global variables
